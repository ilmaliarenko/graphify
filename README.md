...
Fully multimodal. Drop in code, PDFs, markdown, screenshots, diagrams, whiteboard photos, images in other languages, or video and audio files - graphify extracts concepts and relationships from all of it and connects them into one graph. Videos are transcribed with Whisper using a domain-aware prompt derived from your corpus. YAML/YML files (Kubernetes, Kustomize, Helm, config) are indexed for semantic extraction. SQL files are AST-extracted deterministically — tables, views, functions, foreign keys, and FROM/JOIN relationships map directly into the graph with no LLM needed. **dbt models are auto-detected** (parent dir contains `dbt_project.yml`, or file uses Jinja `{{ }}` / `{% %}`) and routed through a Jinja-aware extractor that emits precise `references` edges from `ref()`, `references_source` from `source()`, `uses_var` from `var()`, plus `config()` attributes (alias, materialized) on the file node — `pip install graphifyy[dbt]`. **Airflow / Astronomer Cosmos DAG files are auto-detected** (any `.py` with `from airflow` / `from cosmos` imports) and structurally extracted on top of the standard Python AST: DAGs, tasks (with operator type), `>>` / `chain()` task dependencies, `Variable.get` / `BaseHook.get_connection` references (resolved through module-level constants), `Dataset` declarations, Cosmos `DbtDag` bindings (dbt project path + `RenderConfig(select=..., exclude=...)` selectors — even when assigned to an upstream variable), and `BashOperator(bash_command=f"dbt run --select X --exclude Y")` with concatenated/f-string commands. **Cross-system bridges resolver** runs after all per-file extractors finish and emits `bridges_to` edges between entities that exist in MULTIPLE systems — e.g. an Airflow Cosmos `tag:bookstore` selector → every dbt model whose `config(tags=[...])` contains `bookstore`; a Cosmos DAG bound to a dbt project path → every dbt model whose `source_file` is under that path. Bridge edges are tagged INFERRED with a human-readable `bridge_reason` for audit. **`graphify dbt-manifest <manifest.json>`** converts a dbt `target/manifest.json` (produced by `dbt parse`) into graph nodes/edges using the same ID convention as the file extractors — file-walking nodes and manifest nodes naturally collapse into one merged entity per model. Captures everything dbt resolved at compile time: `database`, `schema`, `materialized`, `alias`, `tags`, plus the full `parent_map` lineage (env-resolved, tighter than what tree-sitter sees from source). **Snowflake admin DDL** (`apply_masking_*.sql`, `create_pipes_*.sql`, `create_grant_*.sql`, etc.) is detected by signature and routed through `extract_snowflake_ddl`: emits nodes for masking policies / pipes / functions / roles + `applies_masking_policy` edges from `ALTER TABLE T MODIFY COLUMN c SET MASKING POLICY P`, plus `grants_to` edges with `snowflake_privileges` and `snowflake_grant_object` attributes for every `GRANT ... TO ROLE` statement. **LLM-memory utilities** treat the persisted graph as queryable memory — `graphify search "<query>"` does fast substring/keyword lookup with 1-hop neighbor expansion; `graphify context "<task>" --budget 2000` builds a token-budgeted subgraph for prompt injection (BFS from query-matching seeds, prioritizes EXTRACTED edges over INFERRED, stops when over budget); `graphify diff --since 24h` shows what's new in memory; `graphify reflect` prints god nodes / recently-added / AMBIGUOUS edges to review / breakdowns by dbt resource type / Snowflake object / Airflow operator. All four are pure-read on `graph.json` and plan-mode-safe. Every node and edge now carries an `extracted_at` (unix seconds) timestamp so recency-aware queries work.  Terraform/HCL files (`.tf`, `.hcl`) are AST-extracted deterministically — resources, modules, data sources, variables, outputs, and their cross-references (`module.x`, `var.y`, `data.x.y`, `local.z`, resource-to-resource) map into the graph with no LLM needed (`pip install graphifyy[terraform]`). 26 file types supported via tree-sitter AST (Python, JS, TS, Go, Rust, Java, C, C++, Ruby, C#, Kotlin, Scala, PHP, Swift, Lua, Zig, PowerShell, Elixir, Objective-C, Julia, Verilog, SystemVerilog, Vue, Svelte, Dart, Terraform/HCL).
...
|------|-----------|------------|
| Code | `.py .ts .js .jsx .tsx .mjs .go .rs .java .c .cpp .rb .cs .kt .scala .php .swift .lua .zig .ps1 .ex .exs .m .mm .jl .vue .svelte .sql .tf .hcl` | AST via tree-sitter + call-graph (cross-file for all languages) + Java extends/implements + docstring/comment rationale. SQL: tables, views, functions, foreign keys, FROM/JOIN edges (requires `pip install graphifyy[sql]`). dbt SQL (auto-detected via `dbt_project.yml` or Jinja markers): `references` from `ref()`, `references_source` from `source()`, `uses_var` from `var()`, `config()` attributes (requires `pip install graphifyy[dbt]`). Terraform/HCL: resources, modules, data sources, variables, outputs, cross-references (requires `pip install graphifyy[terraform]`) |
