...
_DISPATCH: dict[str, Any] = {
    ...
    # .yml / .yaml are NOT auto-classified as code (still in DOC_EXTENSIONS so
    # non-dbt YAML stays in the semantic pipeline), but if a caller explicitly
    # hands one to extract() — e.g. `graphify ast-only` discovering schema YAML
    # in a dbt project — route through the dbt YAML extractor.  Non-dbt YAML
    # extracts as an empty/single-node result, which is the right no-op.
